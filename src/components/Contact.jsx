function Contact() {
  return (
    <section id="contact"  className="bg-white max-w-5xl mx-auto my-12 p-10 rounded-2xl shadow-lg border border-gray-100">
      <h2 className="text-3xl font-bold mb-6 text-slate-800">Contact Me</h2>

  <div className="space-y-4 text-gray-700">

      <p>Email: nikhildamahe26@gmail.com</p>
      <p>Phone: 9021927520</p>

      <p>
        GitHub: 
        <a href="https://github.com/NIKH320"
           className="text-blue-600 hover:underline ml-2"
           target="_blank">
          github.com/NIKH320
        </a>
      </p>

      <p>
        LinkedIn: 
        <a href="https://www.linkedin.com/in/nikhil-mehtapsing-damahe-bb201232b/" 
           className="text-blue-600 hover:underline ml-2"
         target="_blank">
          LinkedIn Profile
        </a>
      </p>

      <p>
        Resume:
           <a 
           href="/Resume_NikhilDamahe_SDE.pdf"
           target="_blank"
           rel="noopener noreferrer"
           className="text-blue-600 hover:underline"
          >
  Download Resume
</a>
      </p>


</div>
      
    </section>
  )
}

export default Contact